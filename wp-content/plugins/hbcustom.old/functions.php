<?php	
	...
	**/
	...
	function hb_custom_header_script(){
	    ...
	    //making psd or ai file upload required for designers
		global $wpdb;
        foreach(get_user_meta(get_current_user_id(), $wpdb->prefix.'capabilities', true) as $key=>$value){
        if($key == 'freelancer'){
	   ?>
					...
					file_frame.on( 'select', function() {
						...
						var ext = [];
				 		jQuery.each(attachment,function(i,f){
				 			...
				 			files_id.push(f.id);
							ext.push(f.filename.split('.').pop());
				 		});
						console.log(jQuery.inArray( "psd", ext));
						if (jQuery.inArray('psd', ext) == -1 && jQuery.inArray('ai', ext) == -1) {
                         alert('Please include psd or ai file in attachment');
						 return;
                         }
...
<?php   }
  ...
  $employer_freelancer = get_role('employer_freelancer');
				    $employer_freelancer->add_cap('upload_files');
	...
	function hb_comment_clarification_board($comment_id){
		...
		if(!empty($files)){
			...
			foreach ($files as $key) {
				 $f = explode("|", $key);
				 if(pathinfo($f[0], PATHINFO_EXTENSION) != "psd" && pathinfo($f[0], PATHINFO_EXTENSION) != "ai"){
				 $attachments .= "<li class='file-extension file-other'><div><img src='".$f[1]."'></div><a href='".$f[1]."' title='".$f[0]."' target='_blank'>".$f[0]."</a></li>";
			    }
				}

			$attachments .= "</ul><div id ='source'><ul class='hb_cb_files' style='list-style-type: none;'>";
				foreach ($files as $key) {
				 $f = explode("|", $key);
				 if(pathinfo($f[0], PATHINFO_EXTENSION) == "psd" || pathinfo($f[0], PATHINFO_EXTENSION) == "ai"){
				 $attachments .= "<li class='file-extension file-other'><a href='".$f[1]."' title='Download psd/ai file' target='_blank'>Download Original Source File</a></li>";
				 update_comment_meta( $comment_id, 'psd', $f[1] );
			    }
				}
               $attachments .= "</ul></div>";
