jQuery(document).ready(function($) {
	/* How it Work ****/	
	var how_it_work = localStorage.getItem("how_it_work");	
	if(how_it_work == 'true')
	{		
		var $href = $('a[title="how_it_work"]').attr('href');	    
	    var $anchor = $($href).offset();		    
	    $('body').animate({ scrollTop: $anchor.top });
	    localStorage.setItem("how_it_work", "false");
	}

	$('a[title="how_it_work"]').on('click', function(){
		...
		if(pathname == '/' ){
			...
			localStorage.setItem("how_it_work", "true");
	...
	/** review confirm**/
	$('.hb_custom_request_confirm').on( 'click', function( e ) {

		var status = $(this).val();		
		
		if ( 'Request Modification' == status ) {			
			$('input[name="project_status"]').val('working');

		} else if( 'Review and Complete Order' == status ) {			
			$('input[name="project_status"]').val('closed_complete');
		}

		$("#manage_project").submit();
	});
});
