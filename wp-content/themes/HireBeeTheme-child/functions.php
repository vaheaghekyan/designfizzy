...
function hide_attachment_details() {
    echo '<style>.media-sidebar{display:none !important;}

	</style>';
}
add_action( 'wp_footer', 'hide_attachment_details' );

// remove media library tab
function remove_medialibrary_tab($strings) {
        if ( !current_user_can( 'administrator' ) ) {
         
			unset($strings['gallery']);
		    unset($strings['library']);
        return $strings;
        }
        else
        {
	...
	function remove_medialibrary_tab1($tabs) {
    if ( !current_user_can( 'administrator' ) ) {
...
add_filter('media_view_strings', 'remove_medialibrary_tab1');
