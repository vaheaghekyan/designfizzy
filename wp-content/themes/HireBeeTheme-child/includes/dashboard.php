...
function hrb_maybe_warn_escrow_info_missing() {
	...
	if ( hrb_is_escrow_enabled() && current_user_can('edit_bids') && ! hrb_escrow_receiver_gateway_fields_valid( $user->ID ) ) {
		...
		//print_r(get_user_meta(get_current_user_id(), 'gyp5_capabilities', true));
	foreach(get_user_meta(get_current_user_id(), 'gyp5_capabilities', true) as $key=>$value){
					if($key != 'employer_freelancer' && $key != 'employer'){
		appthemes_display_notice( 'error',sprintf( __( "Note: Escrow information is missing. You cannot receive payments without that information.<br/>%s.", APP_TD ), $profile_link ) );
...
function hrb_output_active_fdropdown( $posts, $attributes = '' ) {
	$items = array(
		'publish' => __( 'Unselected Projects', APP_TD ),
		'working' => __( 'Selected Projects', APP_TD ),
		'closed_complete' => __( 'Completed Projects', APP_TD ),
	);
 ...
 */
function hrb_output_search_product(){
?>
	<form method="get" class="search_form">		
		<div>
			<input type="text" placeholder="<?php echo __( 'Search', APP_TD ); ?>" name="search_filter" id="s" class="text search" value="<?php esc_attr( hrb_output_search_query_var('ls') ); ?>" />													
		</div>
		<div>
...
function get_the_hrb_dashboard_project_work_actions( $post, $proposal = '' ) {
  ...
  $recipient_id = $dashboard_user->ID == $post->post_author ? $post->post_author  : $post->post_author;
...
function _hrb_dashboard_user_work_actions_atts( $actions, $workspace, $post = '', $recipient = '' ) {
	...
	if ( $recipient ) {
		...
		$atts['review'] = array(
			...
			'title' => __( 'Review', APP_TD ),
			'class' => 'review-user',
			'href' =>  esc_url( get_permalink( $project->ID ) ),
		);
	}
