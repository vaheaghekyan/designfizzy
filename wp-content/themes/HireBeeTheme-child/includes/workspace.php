...
function _hrb_register_workspace_post_type() {
	...
	APP_Item_Registry::register( HRB_WORKSPACE_PTYPE, __( 'Logo Project', APP_TD ) );
}
...
function hrb_get_project_work_ended_statuses() {
	...
	$ended_statuses = array( HRB_PROJECT_STATUS_CANCELED, HRB_PROJECT_STATUS_CLOSED_COMPLETED, HRB_WORK_STATUS_WORKING /*HRB_PROJECT_STATUS_CLOSED_INCOMPLETE*/ );
...
function hrb_p2p_get_participating_posts( $user_id, $args = array(), $post_args = array() ) {
	...
	unset( $post_args['post_status'] );
		...
		}elseif($_GET['filter_status'] == 'closed_complete'){
			$defaults = array(
		'post_type' => HRB_PROJECTS_PTYPE,
		'post__in' => array_keys( $post_ids ),
		'post_status' => 'closed_complete',
		'nopaging' => true,
	);
		}else{
			$defaults = array(
