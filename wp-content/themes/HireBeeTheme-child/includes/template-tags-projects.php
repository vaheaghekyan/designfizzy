...
function the_hrb_project_remain_days( $post_id = 0, $alt_output = false ) {
	...
	if ( 'publish' != get_post_status( $post_id ) ) {
		...
		$days_left = (int) $days;
		$days_left_label = _n( 'hour left', 'hours left', $days, APP_TD );
...
function the_hrb_project_files( $post_id = 0, $before = '', $after = '' ) {
	...
	$attachment_ids1 = get_the_hrb_project_attachments( $post_id );

	$attachment_ids = array();
	global $wpdb;
	$r = $wpdb->get_results("SELECT `id` FROM `{$wpdb->prefix}posts` WHERE `post_parent` = '{$post_id}'", ARRAY_A);

	foreach(array_reverse($r) as $k => $rv){
		$attachment_ids[$k] = (int) $rv['id'];
	}
