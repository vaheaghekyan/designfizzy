...
function the_hrb_page_title_header( $title = '' ) {
	...
	if ( ! $title ) {
		if ( is_singular( HRB_PROJECTS_PTYPE ) ) {
			$title = __( 'Project Details', APP_TD ). ': '.$post->post_title;
		} else {
...
function get_the_hrb_user_nav_links() {
	...
	if ( ! is_user_logged_in() ) {
		...
		$user_links = array(
			...
			'add_project' => array(
				'name' => __( 'Add A Project', APP_TD ),
				'url' => '/post-a-project',
				'class' => '',
				'title' => __( 'Add a Project', APP_TD ),
			),
			'user' => array(
...
?>
	...
	<?php if ( ! empty( $user_link['name'] )):  ?>
		...
		<li class="<?php echo esc_attr( 'hrb-' . $key ); ?>">
			...
			<?php 			
				if($user_link['sub_menu']){				
					_hrb_output_sub_menu($user_link["sub_menu"]);
				}
			?>	
		</li>
...
function _hrb_output_sub_menu( $sub_menu='' ){
	global $current_user;
	
	if($sub_menu == 'project'){
	?>
		<ul class="dropdown-menu">
			<li><?php echo html_link( appthemes_get_edit_profile_url(), __( 'Edit Profile', APP_TD ) ); ?></li>
			<li><?php echo html_link( get_the_hrb_project_create_url(), __( 'Add a Project', APP_TD ) ); ?></li>
			<li><a href="<?php echo esc_url( hrb_get_dashboard_url_for() ); ?>"><?php echo __( 'Activity', APP_TD ); ?></a></li>
			<li><a href="<?php echo esc_url( hrb_get_dashboard_url_for('projects') ); ?>"><?php echo __( 'Projects', APP_TD ); ?></a></li>
			<li><a href="<?php echo esc_url( hrb_get_dashboard_url_for('payments') ); ?>"><?php echo __( 'Payments', APP_TD ); ?></a></li>
			<li><?php echo __( 'Stats', APP_TD ); 

			?>
				<ul class="dropdown-menu">
					<li class="rating">
					   <?php echo __( 'Rating:', APP_TD ); ?>
					   <span><?php the_hrb_user_rating( $current_user, __( 'n/a', APP_TD ) ); ?></span>
					</li>
					<li class="projects-current">
					   <?php echo __( 'Active Projects:', APP_TD ); ?>
					   <span><?php the_hrb_user_related_active_projects_count( $current_user ); ?></span>
					</li>
					<li class="projects-complete">
					   <?php echo __( 'Projects Completed:', APP_TD ); ?>
					   <span><?php the_hrb_user_completed_projects_count( $current_user ); ?></span>
					</li>
					<li class="reviews-received">
					   <?php echo __( 'Reviews Received:', APP_TD ); ?>
					   <span><?php the_hrb_user_total_reviews( $current_user ); ?></span>
					</li>
					<li class="reviews-given">
					   <?php echo __( 'Reviews Given:', APP_TD ); ?>
					   <span><?php the_hrb_user_total_authored_reviews( $current_user ); ?></span>
					</li>
					<li class="reviews-success_rate">
					   <?php echo __( 'Success Rate:', APP_TD ); ?>
					   <span><?php the_hrb_user_success_rate( $current_user ); ?></span>
					</li>
				</ul>	
			</li>			
		</ul>
