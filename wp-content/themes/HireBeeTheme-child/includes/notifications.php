...
add_action( 'hrb_new_project1', 'hrb_new_project_notify_designer', 10, 1 );
...
/**
 * Notify DESIGNERS on new posted projects.
 * Email Only
 */
function hrb_new_project_notify_designer( $post_id ) {
	global $hrb_options;
	...
	$users = get_users('role=freelancer');
foreach( $users as $user ){
   
	appthemes_send_email( $user->user_email, $subject, $content );
}
...
*/
	$retained = appthemes_escrow_receiver_amount($order->get_total());
/*
