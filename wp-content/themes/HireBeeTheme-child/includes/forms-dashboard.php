...
class HRB_Workspace_Form_Manage extends APP_View {
    ...
    function process( $workspace_id, $project_id, $user_id ) {
       ...
       // print_r($_POST);
		...
		//delete psd/ai from comment after customer request modification
		
		if($_POST['project_status'] == 'working'){
		    $args = array(
	'number' => 1,
	...
	$content = strstr($content,"<div id ='source'>",true);
	$commentarr = array();
    $commentarr['comment_ID'] = $comment_id;
    $commentarr['comment_content'] = $content;
    wp_update_comment( $commentarr );
	$file = get_comment_meta( $comment_id, 'psd', true );
	$path = str_replace(rtrim(get_site_url(),'/').'/', ABSPATH, $file);
	unlink($path);
	endforeach;
		}
...
class HRB_Workspace_Form_Review extends APP_View {
	...
	function parse_query( $wp_query ) {
        ...
        wp_redirect( esc_url( get_permalink( $project_id ) ) );
        exit;
	}
