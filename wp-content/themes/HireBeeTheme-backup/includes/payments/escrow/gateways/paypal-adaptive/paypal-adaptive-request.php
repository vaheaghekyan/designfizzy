...
class APP_PayPal_Adaptive_Request {
	...
	protected static $nvp_endpoint_url = array(
	...
	public function execute_payment( APP_Escrow_Order $order, $pay_key ) {
		...
		foreach( $receivers as $user_id => $amount ) {
			...
			$data['L_EMAIL'.$key] = $paypal_email;
			$data['L_AMT'.$key] = (float)$amount;

			$key++;
		}

		return $this->requestNVP( $order, $data );
	}
	...
	protected function requestNVP( APP_Escrow_Order $order, $data ) {
		...
		curl_setopt( $ch, CURLOPT_URL, $this->options['nvp_endpoint'] );
		...
		$response = $this->deformatNVP( curl_exec( $ch ) );
	...
	protected function set_options( $options  ) {
		...
		$all_options = array(
			...
			'nvp_endpoint' => self::$nvp_endpoint_url[ $gateway_mode ],
		);
