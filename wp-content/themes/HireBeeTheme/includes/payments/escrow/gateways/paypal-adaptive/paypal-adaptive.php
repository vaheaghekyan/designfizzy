...
class APP_PayPal_Adaptive extends APP_PayPal implements APP_Escrow_Payment_Processor {
	...
	protected function _process_escrow( APP_Escrow_Order $order, $options ) {
		...
		$return_url = htmlspecialchars_decode( $this->get_return_url( $order ) );
		$cancel_url = htmlspecialchars_decode( $this->get_cancel_url( $order ) );
	...
	public function complete_escrow( APP_Escrow_Order $order, array $options ) {
		...
		$responsecode = strtoupper( $pay_response['ACK'] );
			...
			<table id="admin-escrow-order-details">
				...
				<tbody>
						...
						<tr>
							...
							<td><?php echo appthemes_display_price( $order->get_total() - appthemes_escrow_receiver_amount($order->get_total()), $order->get_currency() ); ?> <?php echo __( '(when completed)', APP_TD ); ?> </td>
						</tr>
					</tr>
				</tbody>
			</table>
